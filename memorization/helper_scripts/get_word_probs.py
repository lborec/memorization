...
def get_word_probabilities(model, tokenizer, texts):
    ...
    for text in texts:
        text = " " + text[1]
        tokens = tokenizer.encode(text, add_special_tokens=True, truncation=True, max_length=512, padding=False)
...
sampled_duplicates = parse_json_file("memorization/dataset/stats/train_stats/duplicates.json", [30])
